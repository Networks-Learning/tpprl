...
python rq-compare.py "${in_file}" "${user_idx}" "${out_dir}" --N $N --q $q --reward top_k_reward --until ${until} --epochs ${epochs} --restore --only-cpu --reward-top-k $K --save-every $save_every
