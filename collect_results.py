...
def worker_user(params):
    (user_idx, output_dir, test_batches, RQ_cap_adjust, for_epoch, verbose) = params
...
@click.option('--for-epoch', 'for_epoch', help='Whether to produce the CSV for a given epoch. Negative numbers of running it on the latest epoch.', default=-1)
...
def run(output_dir, save_csv, tweeters_data_file, batches, force, RQ_cap_adjust, for_epoch, parallel, verbose):
    ...
    """
    cmd(output_dir, save_csv, tweeters_data_file, batches, force, RQ_cap_adjust, for_epoch, parallel, verbose)


def cmd(output_dir, save_csv, tweeters_data_file, batches, force, RQ_cap_adjust, for_epoch, parallel, verbose):
    ...
    all_params = [(x, output_dir, batches, RQ_cap_adjust, for_epoch, verbose) for x in user_idxes]
    if parallel:
